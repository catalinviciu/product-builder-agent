# Product Agent — Setup Guide for Google Antigravity

This guide helps you set up Google Antigravity to work with Product Agent alongside your own codebase. By the end, Antigravity will understand your product context and have a story map bootstrapped for your project.

> **Using Claude Code instead?** See [SETUP.md](./SETUP.md).

## Prerequisites

- [Google Antigravity](https://antigravity.google) installed and working
...
└── .agent/                  # Antigravity config (created during setup)
    ├── rules/
    │   └── product-agent.md
    └── skills/
        └── story-map-updater/
...
## Step 1: Create your Antigravity rules file

Antigravity reads project context from `.agent/rules/` in the workspace root. Create the folder structure and rules file by opening Antigravity in the workspace root and pasting this prompt:

```
Create the file `.agent/rules/product-agent.md` at the workspace root. It should include:
...
2. A section about Product Agent with this content:
   ...
   Reusable agent skills live in `ProductSkills/` and are also installed at `.agent/skills/`. Key skill:

   - **story-map-updater** — Keeps a Jeff Patton-style User Story Map in sync as features are planned and shipped. Read the SKILL.md file for full instructions on when and how to update the story map.
   ...
   - **Regenerate:** Run the generate script after editing the JSON (see Step 3 below)
...
3. Any existing rules files should be preserved — don't overwrite other files in `.agent/rules/`.
```
...
## Step 2: Install the story-map-updater skill

Antigravity loads skills from `.agent/skills/` in your workspace. Copy the story-map-updater skill so Antigravity can find it:

**Option A: Copy the folder**

```bash
mkdir -p .agent/skills
cp -r "ProductSkills/story-map-updater" .agent/skills/
```

**Option B: Symlink (keeps it in sync with the source)**

```bash
mkdir -p .agent/skills
# macOS / Linux
ln -s "../../ProductSkills/story-map-updater" .agent/skills/story-map-updater

# Windows (run as administrator)
mklink /D ".agent\skills\story-map-updater" "..\..\ProductSkills\story-map-updater"
```

After this, Antigravity will automatically load the skill when you ask it to update the story map.

---

## Step 3: Bootstrap Your Story Map

> **No codebase yet?** If your `<YOUR_CODEBASE>/` folder is empty or doesn't exist yet, **skip this step entirely**. You can come back and bootstrap the story map later once you have code to map. Just complete Steps 1, 2, and 4 for now.

The story map captures your application's user journeys. To generate the initial version, paste this prompt into Antigravity:

```
I want to create a Jeff Patton-style User Story Map for my codebase at `./<YOUR_CODEBASE>/`.

1. Read the story-map-updater skill (`.agent/skills/story-map-updater/SKILL.md`) to understand the story map format and quality criteria.
...
## Step 4: Set Your Code Path in Product Agent
...
This ensures the Solution Copy Anchor generates prompts that point Antigravity to the right folder.
...
3. **Paste into Antigravity** — it will receive the full context (Opportunity, Solution, persona, codebase path) and can plan the implementation
4. **Story map stays in sync** — the prompt instructs Antigravity to use the story-map-updater skill, so your story map evolves with each feature
...
| Antigravity doesn't know about Product Agent | Verify `.agent/rules/product-agent.md` exists and contains the Product Agent section from Step 1 |
| Antigravity can't find the skill | Verify `.agent/skills/story-map-updater/SKILL.md` exists (see Step 2) |
