...
|  | View the predefined Story Map config (JSON path, MD path, generate script) in Settings — read-only, ProductAgent only | `ProductLineSettingsView` · `AccordionSection` |
...
| **Add a single story to a task cell** | Click the '+' affordance in a task cell to create a new story inline without re-running the full slicer | `PattonMap` · `AddStoryCell` · `addStoryToCell` |
...
|  | Switch between persona-specific backbones via the persona strip when a solution covers multiple personas | `StoriesTab` · `PersonaStrip` · `listPersonasWithStories` |
|  | See a story count next to each iteration label to size the slice at a glance | `PattonMap` |
...
|  | Navigate to the previous or next story in map reading order (WS → ENH → GA, left-to-right within each row) using ← / → buttons in the footer or ArrowLeft / ArrowRight keyboard shortcuts, staying within the active persona | `StoryDetailSlideOver` · `navigateStoryDetail` · `orderStoriesForTraversal` |
|  | Share a direct link to an open story via the ?story=<id> URL param — pasting the URL in a fresh tab opens the panel to that exact story | `StoryDetailSlideOver` · `useSearchParams` · `history.replaceState` |
| **Hand off a single story to the agent** | Click 'Plan & Implement story' in the slide-over footer — if codebase path and design system are configured, copies a story-scoped planning prompt with resolved settings values (no placeholders); otherwise redirects to Settings with a banner listing the missing fields | `StoryDetailSlideOver` · `buildPlanImplementStoryPrompt` · `runGatedPlanImplement` · `ProductLineSettingsView` · `Toaster` |
...
| **Add acceptance criteria + analytics events to stories** | Click the toolbar 'Add acceptance criteria' button — if an analytics platform is configured, copies the AC writer prompt with the platform's event-tracking syntax resolved from product-line settings; otherwise redirects to Settings with a banner listing the missing analytics-platform field | `StoryMapToolbar` · `StoriesTab` · `buildUserStoryAcWriterPrompt` · `settings-redirect` · `openSettingsWithRedirect` |
